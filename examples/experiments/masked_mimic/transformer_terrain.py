...
"""
MaskedMimic on Complex Terrain (transformer)
============================================

Same MaskedMimic distillation as masked_mimic/transformer.py, but trained on
procedurally varied terrain so the distilled policy is terrain-aware.

Differences vs masked_mimic/transformer.py:
  - terrain_config() returns ComplexTerrainConfig (matches the smpl-terrains expert).
  - The "terrain" heightmap obs (auto-injected by BaseEnv as obs["terrain"]) is fed
    into the transformer in three places:
      * encoder : normalized via terrain_norm_enc, concatenated into the trunk MLP
      * prior   : a separate terrain_token modality fed to the transformer
                  (paper-faithful: terrain has its own encoder/token; this is the
                  branch used at inference)
      * trunk   : normalized via terrain_norm_trunk, concatenated into the actor MLP
    so the student sees the ground just like the expert (smpl-terrains) does.

Expert (Stage 1) is the pretrained complex-terrain SMPL motion tracker:
    --expert-model-path data/pretrained_models/motion_tracker/smpl-terrains/last.ckpt
"""
...
def additional_experiment_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Inference only: drive the character in real time via keyboard "
        "(arrows=move dir, A/D=heading, W/S=speed). Requires headed mode "
        "(--headless False).",
    )


def terrain_config(args: argparse.Namespace):
    """Build terrain configuration — complex (varied) terrain, matches the expert."""
...
def agent_config(
    ...
    # TERRAIN: normalize obs["terrain"] -> terrain_norm_enc and feed into the trunk MLP.
    encoder_config = ModuleContainerConfig(
        in_keys=["max_coords_obs", "mimic_target_poses", "masked_mimic_target_poses", "masked_mimic_target_bodies_masks", "masked_mimic_target_times", "masked_mimic_target_poses_masks", "terrain"],
        ...
        models=[
            ...
            # Terrain heightmap normalizer (added for terrain-aware MaskedMimic)
            ObsProcessorConfig(
                in_keys=["terrain"],
                out_keys=["terrain_norm_enc"],
                normalize_obs=True,
                norm_clamp_value=5,
                module_operations=[ModuleOperationForwardConfig()],
            ),
            ...
            MLPWithConcatConfig(
                in_keys=["max_coords_obs_norm", "mimic_target_poses_norm", "masked_mimic_target_poses_norm", "masked_mimic_target_bodies_masks", "masked_mimic_target_times_norm", "masked_mimic_target_poses_masks", "terrain_norm_enc"],
    ...
    # TERRAIN (paper-faithful): obs["terrain"] gets its own encoder -> terrain_token,
    # which is fed to the transformer as a separate modality token. This is the branch
    # actually used at inference.
    prior_config = ModuleContainerConfig(
        in_keys=["max_coords_obs", "masked_mimic_target_poses", "masked_mimic_target_masks", "masked_mimic_target_times", "masked_mimic_target_poses_masks", "historical_pose_obs", "terrain"],
        ...
        models=[
            ...
            # Terrain heightmap as its own modality token (paper-faithful):
            # a unique terrain encoder -> a single token fed to the transformer,
            # so the transformer can attend to terrain independently.
            MLPWithConcatConfig(
                in_keys=["terrain"],
            ...
            TransformerConfig(
                in_keys=["current_state_token", "terrain_token", "masked_mimic_target_poses_token", "historical_pose_obs_token", "masked_mimic_target_poses_masks"],
    ...
    # TERRAIN: normalize obs["terrain"] -> terrain_norm_trunk and feed into the actor MLP
    # so the produced actions account for the ground (footstep placement).
    trunk_config = ModuleContainerConfig(
        in_keys=["max_coords_obs", "previous_actions", "terrain", "vae_latent"],
        ...
        models=[
            ...
            MLPWithConcatConfig(
                in_keys=["max_coords_obs_norm", "previous_actions_norm", "terrain_norm_trunk", "vae_latent"],
                out_keys=["actor_trunk_out"],
...
def apply_inference_overrides(
    ...
    # Stage 3: real-time keyboard steering — swap the masked_mimic control for the
    # keyboard-driven variant (root-only conditioning, target from user input).
    if getattr(args, "keyboard_steering", False) and env_cfg is not None:
        import dataclasses
        from protomotions.envs.control.masked_mimic_keyboard_control import (
            MaskedMimicKeyboardControlConfig,
        )

        old = env_cfg.control_components.get("masked_mimic")
        if old is not None:
            carried = {
            ...
            env_cfg.control_components["masked_mimic"] = (
                MaskedMimicKeyboardControlConfig(**carried)
            )
