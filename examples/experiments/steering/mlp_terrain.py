...
"""
Steering on Complex Terrain (AMP)
=================================

Same steering task as steering/mlp.py (walk/run toward a target movement
direction at a target speed, while facing a target heading) with AMP style
priors, but trained on procedurally varied terrain (slopes + stairs).

User-controllable signals at inference (steering control component):
  - tar_dir      : target movement direction (2D)
  - tar_face_dir : target facing / heading (2D, decoupled from tar_dir -> strafing)
  - tar_speed    : target speed along tar_dir

Differences vs steering/mlp.py:
  - terrain_config() returns ComplexTerrainConfig (non-flat).
  - "terrain" heightmap obs (auto-injected by BaseEnv as obs["terrain"]) is added
    to the actor / critic / disc_critic in_keys so the policy can see the ground.
  - The AMP discriminator is left UNCHANGED (no terrain): it only judges motion
    style from historical body state, and reference motions have no terrain obs.
"""


def terrain_config(args: argparse.Namespace):
    """Build terrain configuration — complex (varied) terrain."""
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    # Control components - steering manages task state (direction/speed/facing)
    control_components = {
        "steering": SteeringControlConfig(),
    }

    # Observation components configuration.
    # NOTE: the "terrain" heightmap observation is added automatically by BaseEnv
    # (obs["terrain"]); it does NOT need to be declared here, only referenced in
    # the agent's in_keys below.
    observation_components = {
        ...
        # [local tar_dir(2), tar_speed(1), local tar_face_dir(2)]
...
def agent_config(
    ...
    # actor/critic see steering obs + terrain heightmap; the discriminator only
    # uses historical body state (NO terrain / NO steering).
    actor_config = PPOActorConfig(
        num_out=robot_config.kinematic_info.num_dofs,
        actor_logstd=-2.9,
        in_keys=["max_coords_obs", "steering", "terrain", "historical_max_coords_obs"],
    ...
    # Discriminator only sees historical body state (NOT steering / terrain obs).
    # Reference (expert) motions have no terrain obs, so adding terrain here would
    # break AMP. Leave this exactly as in the flat steering config.
    ...
    # Disc critic: value network for discriminator rewards (terrain-aware).
    disc_critic_config = ModuleContainerConfig(
        in_keys=["max_coords_obs", "terrain", "historical_max_coords_obs"],
        out_keys=["disc_value"],
        models=[
            MLPWithConcatConfig(
                in_keys=["max_coords_obs", "terrain", "historical_max_coords_obs"],
    ...
    reference_obs_components = {
        "historical_max_coords_obs": MdpComponent(
            ...
            static_params={"num_state_history_steps": 8},  # must match EnvConfig.num_state_history_steps (agent injects motion_lib/ids/times/dt)
        ),
    }
